...
namespace part_b{
  ...
  transducer_t your_classifier(const vector<sentence_t> &training_set, const vector<symbol_t> &postags,
    ...
    auto postag_onehot2 = make_onehot(postags);
    auto postag_onehot3 = make_onehot(postags);
    ...
    return compose(group(embedding_lookup, embedding_lookup2, postag_onehot, postag_onehot2, postag_onehot3), concatenate, dense0, dense1, onehot_inverse);
  }
  ...
  get_features(const vector<symbol_t> &sentence, const vector<symbol_t> &postags, unsigned target_index) {
    ...
    if (target_index > 0) {
      if (target_index < sentence.size() - 1) {
        return vector<feature_t>{sentence[target_index], sentence[target_index + 1], postags[target_index - 1], postags[target_index], postags[target_index + 1]};
      } else {
        return vector<feature_t>{sentence[target_index], ""<S>"", postags[target_index - 1], postags[target_index], ""<S>""};
      }
    } else {
      if (sentence.size() > 1) {
        return vector<feature_t>{sentence[target_index], sentence[target_index + 1], ""<S>"", postags[target_index], postags[target_index + 1]};
      } else {
        return vector<feature_t>{sentence[target_index], ""<S>"", ""<S>"", postags[target_index], ""<S>""};
      }
    }
