...
namespace part_a{
  ...
  transducer_t your_classifier(const vector<sentence_t> &training_set, const vector<symbol_t> &postags,
    ...
    return compose(group(postag_onehot, postag_onehot2, postag_onehot3), concatenate, dense0, dense1, onehot_inverse);
  }
  ...
  get_features(const vector<symbol_t> &sentence, const vector<symbol_t> &postags, unsigned target_index) {
	...
	return vector<feature_t>{postags[target_index - 1], postags[target_index], postags[target_index + 1]};
      } else {
	return vector<feature_t>{postags[target_index - 1], postags[target_index], "<S>"};
      }
    } else {
      if (sentence.size() > 1) {
	return vector<feature_t>{"<S>", postags[target_index], postags[target_index + 1]};
      } else {
	return vector<feature_t>{"<S>", postags[target_index], "<S>"};
      }
    }
