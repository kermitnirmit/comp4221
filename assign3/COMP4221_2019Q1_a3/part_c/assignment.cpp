  ...
  transducer_t your_classifier(const vector <sentence_t> &training_set, const vector <symbol_t> &postag_vocab,
    ...
    return compose(
      group(
        ...
        embedding_lookup2,
        embedding_lookup3,
         iobes_syntactic_onehot), // handles the input - target token POS tag
      concatenate,
  ...
  vector<feature_t> get_features(const vector<token_t> &sentence, const vector<symbol_t> &postags,
    ...
    if (target_index > 0) {
      if (target_index < sentence.size() - 1) {
        return vector<feature_t>{sentence[target_index - 1], sentence[target_index], sentence[target_index + 1], shallow_syntactic_tags[target_index]};
        // return vector<feature_t>{sentence[target_index], sentence[predicate_index], postags[target_index - 1], postags[target_index], postags[target_index + 1], shallow_syntactic_tags[target_index], (double) target_index - predicate_index};
      } else {
        return vector<feature_t>{sentence[target_index - 1], sentence[target_index], "<S>", shallow_syntactic_tags[target_index]};
        // return vector<feature_t>{sentence[target_index], sentence[predicate_index], postags[target_index - 1], postags[target_index], "<S>", shallow_syntactic_tags[target_index], (double) target_index - predicate_index};
      }
    } else {
      if (sentence.size() > 1) {
        return vector<feature_t>{"<S>", sentence[target_index], sentence[target_index + 1], shallow_syntactic_tags[target_index]};
        // return vector<feature_t>{sentence[target_index], sentence[predicate_index], "<S>", postags[target_index], postags[target_index + 1], shallow_syntactic_tags[target_index], (double) target_index - predicate_index};
      } else {
        return vector<feature_t>{ "<S>", sentence[target_index], "<S>", shallow_syntactic_tags[target_index]};
        // return vector<feature_t>{sentence[target_index], sentence[predicate_index], "<S>", postags[target_index], "<S>", shallow_syntactic_tags[target_index], (double) target_index - predicate_index};
      }
    }
  }
}
